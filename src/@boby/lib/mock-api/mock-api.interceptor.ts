...
{
    const defaultDelay = inject(BOBY_MOCK_API_DEFAULT_DELAY);
    const bobyMockApiService = inject(BobyMockApiService);

    // Try to get the request handler
    const {
        handler,
        urlParams,
    } = bobyMockApiService.findHandler(request.method.toUpperCase(), request.url);
